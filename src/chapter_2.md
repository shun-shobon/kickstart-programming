...
### 実行しよう
ウィンドウの三角形の再生マークのボタンがありますね．クリックすると実行されます．新しく画面(ウィンドウ)が表示されました．
...
さて，お気づきの方もいるかもしれませんがこの```size(数字,数字);```というのは画面のサイズを指定しています．もっと言うと，```size(x,y);```というコードは画面の横の長さをx，縦の長さをyにする，という意味を持っています．
...
表にまとめてありますが，関数を覚える必要はありません．なぜなら，リファレンスをみればわかるからです．
...
#### 演習2-3
>[ 演習2-2 ]( #演習2-2 )のコードに書き足して，で描いた正方形の内側にぴったり接する円を描け．
